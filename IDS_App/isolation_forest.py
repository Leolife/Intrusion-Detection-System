...
model = IsolationForest(contamination=0.01)  # 1% to try and reduce false positives, this is subject to change
model.fit(cpu_data_resampled[['cpu_usage']])


def monitor_cpu_usage(model, threshold):  # threshold for a hard-coded % to take a snapshot
    new_data = []
    while True:
...
def retrain_model(new_data, model):
    ...
    combined_data = pd.concat([cpu_data_resampled, new_data.resample('1T').mean()])
...
monitor_cpu_usage(model, threshold=95)